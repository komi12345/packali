                    ...
                    <form action="{{ route('admin.promotions.update', $promotion) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="pack_alimentaire_id" name="pack_alimentaire_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:text-gray-200">
                                    @foreach($packsAlimentaires as $pack)
                                        <option value="{{ $pack->id }}" {{ old('pack_alimentaire_id', $promotion->pack_alimentaire_id) == $pack->id ? 'selected' : '' }}>{{ $pack->nom }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="prix_promotionnel" id="prix_promotionnel" value="{{ old('prix_promotionnel', $promotion->prix_promotionnel) }}" step="0.01" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-gray-200">
                            ...
                            <div>
                                ...
                                <input type="date" name="date_debut" id="date_debut" value="{{ old('date_debut', $promotion->date_debut) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-gray-200">
                                @error('date_debut')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="date" name="date_fin" id="date_fin" value="{{ old('date_fin', $promotion->date_fin) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-gray-200">
                                @error('date_fin')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
